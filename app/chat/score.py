...
def random_component_by_score(component_type, component_map):
    ...
    # From Redis get the hash containing the sum total of scores for the given component type
    values = client.hgetall(f"{component_type}_score_values")
    # From redis, get the hash containing the number of times each component type has been scored
    counts = client.hgetall(f"{component_type}_score_counts")
    # Get all the valid component names from the map
    names = component_map.keys()
    # Loop over those valid names and use them to Ccalculate the average score for each component type
    avg_scores = {}
    for name in names:
        score = int(values.get(name, 1))
        count = int(counts.get(name, 1))
